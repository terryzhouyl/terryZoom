	...
	$("[reg],[url]:not([reg]),[tip]").hover(
		function(e) {
			if($(this).attr('tip') != undefined){
				...
				$('body').append( '<p id="vtip"><img id="vtipArrow" src="../../resource/system/admin/images/vtip_arrow.png"/>' + $(this).attr('tip') + '</p>' );
...
function freeValidate(formvalue){
	var isValidatess = true;
	$(formvalue).find("[reg],[url]:not([reg])").each(function(){
		if(!validate($(this))){
			isValidatess= false;
		}
	});
